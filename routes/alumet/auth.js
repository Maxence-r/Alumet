    ...
    try {
        ...
        if (user.suspended.reason) {
            return res.status(403).json({
                error: "Your account has been suspended. Please check your email for more information.",
            });
        }
        if (user.isA2FEnabled) {
            ...
            res.cookie('token', token, authCookieOptions(sessionMaxAge))
                .status(200)
                .json({
...
router.post('/signup', rateLimit(1), validateAccount, async (req, res) => {
    ...
    try {
        ...
        res.status(400).json({ message: err.message || 'Inscription impossible' });
    }
});
...
router.post('/resetpassword', rateLimit(3), authorizeA2F, async (req, res) => {
    try {
        if (req.body.password.length < 6) {
            return res.status(400).json({ error: 'The password must contain at least 6 characters.' });
        }

...
router.post('/:alumet', validateObjectId, alumetItemsAuth, (req, res) => {
    if (!req.connected || req.alumetObj.owner.toString() !== req.user.id) return res.status(401).json({ error: 'Vous n\'avez pas les permissions pour effectuer cette action !' });
...
router.get('/:alumet', alumetItemsAuth, anonymeAuthentification, validateObjectId, (req, res) => {
    if (!req.connected && !req.auth) return res.status(404).json({ error: 'Vous n\'avez pas les permissions pour effectuer cette action !' })
    if (req.connected && !req.auth && req.alumetObj.owner.toString() !== req.user.id) return res.status(401).json({ error: 'Vous n\'avez pas les permissions pour effectuer cette action !' });

...
const postLayer = async (req, res, next) => {
    try {
        ...
        if (!req.logged && !req.auth) return res.status(401).json({ error: 'Unauthorized x000' });

        if (req.logged) {
            req.ownerId = req.user.id;
            req.ownerType = 'teacher';
            if (alumet.owner !== req.user.id) return res.status(401).json({ error: 'Unauthorized' });
            if (wall.alumet !== alumet._id.toString()) return res.status(401).json({ error: 'Unauthorized x001' });
        }
        ...
        if (req.auth && !req.logged) {
            ...
            if (alumet._id.toString() !== req.alumet.id) return res.status(401).json({ error: 'Unauthorized x002' });
            if (wall.alumet !== req.alumet.id) return res.status(401).json({ error: 'Unauthorized x003' });
            if (wall.post === false) return res.status(401).json({ error: 'Unauthorized x004' });
        }
